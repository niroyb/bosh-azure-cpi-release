...
function notice() {
  echo "
    ...
    3. the script will try to collect error logs including
        a. bosh error logs
        b. job error logs
        c. kernel and syslog of the failed vm
...
function collect_bosh_logs() {
  ...
  if [[ ${bosh_version} == "v1" ]]; then
    output=$(${bosh_cmd} tasks recent |grep error | head -n ${err_log_count})
    tasks=$(echo "${output}" |cut -d '|' -f 2)
  elif [[ ${bosh_version} == "v2" ]]; then
    output=$(${bosh_cmd} tasks -r |grep error | head -n ${err_log_count})
...
function collect_system_logs() {
  ...
  copy_syslog_script="sudo su -c 'rm ${tgz}; mkdir -p $(dirname ${tgz}); cd /var/log/; tar -czf ${tgz} kern.log syslog waagent.log; chmod -R 755 ${tgz}'"
  # for v1
  if [[ ${bosh_version} == "v1" ]]; then
    for vm in ${err_vms}; do
